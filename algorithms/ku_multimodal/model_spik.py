...
class Spikformer(nn.Module):
    def __init__(self,
                 img_size_h=64, img_size_w=64, patch_size=16, in_channels=1, num_classes=11,
                 embed_dims=[64, 128, 256], num_heads=[1, 2, 4], mlp_ratios=[4, 4, 4], qkv_bias=False, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[6, 8, 6], sr_ratios=[8, 4, 2], T = 4, activate_classifier=False
                #  input_method='sum'
                 ):
        super().__init__()
        self.T = T  # time step
        ...
        self.depths = depths
        self.activate_classifier = activate_classifier
    ...
    def forward(self, x):
        ...
        if self.activate_classifier==True:
            x = x.mean(2).mean(0)
            x = self.head(x)
        return x
